  ...
  const filteredRows = useMemo(() => {
    ...
    const takeUnique = (items: MovieItem[], limit = 12) => {
      ...
      for (const it of items) {
        ...
        if ((it.voteAverage || 0) < 6.5) continue;
        out.push(it);
        used.add(it.id);
    ...
    if (tab === "movies") return [{ title: "Popular Movies", items: takeUnique(global, 24) }];
    if (tab === "tv") return [{ title: "Top TV Series", items: takeUnique(trending, 24) }];
    if (tab === "kids") {
            ...
            {featured && (
              <section className="relative mb-8 overflow-hidden rounded-[28px] border border-zinc-800 bg-zinc-900 shadow-2xl">
                ...
                <div className="relative flex min-h-[320px] flex-col justify-end p-5 sm:p-8 md:min-h-[420px] md:p-10">
                  ...
                  <p className="mt-2 max-w-xl text-sm text-zinc-200 sm:text-base">{featured.overview || "Discover a cinematic experience with premium entertainment for every mood."}</p>
