...
export async function POST(request: NextRequest) {
  ...
  if (!email || !name) return NextResponse.json({ error: "Invalid credentials" }, { status: 400 });

  const account = getOrCreateAccount(email, name, picture);
  if (account.isBlocked) return NextResponse.json({ error: "Account blocked" }, { status: 403 });
